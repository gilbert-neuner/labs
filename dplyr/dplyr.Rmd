...
<li>Commas are used when you want to subset by one condition AND another condition. If you want to subset by one condition OR another condition, use the vertical bar `|`.</li>
...
<li>Let's use `filter` to remove rows whose entries for `height`, `weight`, `grade`, `gender`, or `depressed` are `NA`. Let's also remove rows whose entries for `grade` are "Ungraded or other grade".</li>

<li>The `is.na` function returns `TRUE` whenever the argument is `NA` and `FALSE` otherwise.</li>

```{r}
# none of the entries in height, weight, grade, gender or depressed are NA
cdc_complete <- cdc %>%
  filter(!is.na(height), !is.na(weight), !is.na(grade), !is.na(gender), !is.na(depressed)) %>%
...
# lets assign to cdc_complete instead of df because we will use the bmi column later
